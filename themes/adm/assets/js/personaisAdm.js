...
function renderPersonais(listPersonais){
    const ul = document.querySelector(".trainer-list")
    ul.innerHTML = ``;
    listPersonais.forEach((personal) => {
        const newLi = document.createElement("li");
        newLi.innerHTML = `Personal: ${personal.name} (Especialidade: ${personal.specialty}) - <button class="btnDelete" data-id="${personal.id}">Delete</button> / <button class="btnUpdate" data-id="${personal.id}">Alterar</button>`;
        ul.appendChild(newLi);
    });
...
try {
    const listPersonais = await api.getAllPersonals();
    renderPersonais(listPersonais);
} catch (error) {
...
function Update(IdUpdate, e) {
    ...
    document.getElementById("updateForm").onsubmit = async function(event) {
        ...
        const updatedPersonalData = {
            personalId: document.querySelector("#personalId").value, // Corrigido: personalId
        ...
        const updatedPersonalDataStr = JSON.stringify(updatedPersonalData)

        // Debugging: verificar valores antes de enviar
        console.log("Dados a serem enviados:", updatedPersonalDataStr);

        try {
            await api.updatePersonal(updatedPersonalDataStr);
            // Fecha o diálogo
            modal.close(); // Fecha o diálogo
            // Atualiza a lista de personais após o update
            renderPersonais(await api.getAllPersonals());
        } catch (error){
            console.error('Erro na requisição:', error);
        }
}
